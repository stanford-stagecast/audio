...
ClientWriter::ClientWriter( const Address& server, const LongLivedKey& key )
  : server_( server )
  , long_lived_crypto_( key.key_pair().uplink, key.key_pair().downlink, true )
  , next_key_request_( steady_clock::now() )
{}

void ClientWriter::process_keyreply( const Ciphertext& ciphertext )
{
  /* decrypt */
  Plaintext plaintext;
...
void ClientWriter::send_packet( UDPSocket& socket )
{
  if ( session_.has_value() ) {
    session_.value().connection.send_packet( socket );
  }
}

void ClientWriter::receive_packet( const Ciphertext& ciphertext )
{
  if ( session_.has_value() ) {
    session_.value().connection.receive_packet( ciphertext );
  }
}

void ClientWriter::mix_and_encode( const AudioBoard& board, const uint64_t cursor_sample )
{
  while ( mix_cursor_ + opus_frame::NUM_SAMPLES <= cursor_sample ) {
    ...
    for ( uint8_t channel_i = 0; channel_i < board.num_channels(); channel_i++ ) {
      ...
      const auto [gain_into_1, gain_into_2] = board.gain( channel_i );
      for ( uint16_t sample_i = 0; sample_i < opus_frame::NUM_SAMPLES; sample_i++ ) {
        const float value = other_channel[sample_i];
        const float orig_1 = ch1_target[sample_i];
        const float orig_2 = ch2_target[sample_i];
    ...
    session_.value().connection.push_frame( encoder_ );
