...
StatsPrinterTask::StatsPrinterTask( const shared_ptr<AudioDeviceTask> device,
                                    const vector<shared_ptr<NetworkEndpoint>>& network_endpoints,
                                    const shared_ptr<EventLoop> loop )
  : device_( device )
  , network_endpoints_( network_endpoints )
  , loop_( loop )
  ...
  , next_stats_reset( steady_clock::now() + stats_reset_interval )
{
  loop_->add_rule(
    ...
    [&] {
      ...
      for ( const auto& n : network_endpoints_ ) {
        n->generate_statistics( ss_ );
      }
      ss_ << "\n";
      loop_->summary( ss_ );
      ss_ << "\n";
